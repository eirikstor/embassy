...
impl<'d> Output<'d> {
    ...
    pub fn set_high(&mut self) {
        GPIO.set0().write(|w| w.set_setp(self.pin.bit()));
    }

    pub fn set_low(&mut self) {
        GPIO.clr0().write(|w| w.set_clrp(self.pin.bit()));
    }

    pub fn toggle(&mut self) {
        GPIO.not0().write(|w| w.set_notp(self.pin.bit()));
    }
...
impl<'d> Flex<'d> {
    ...
    pub fn set_pull(&mut self, pull: Pull) {
        self.pin.pio().modify(|w| match pull {
            Pull::None => w.set_mode(Pio00Mode::INACTIVE),
            Pull::Up => w.set_mode(Pio00Mode::PULL_UP),
            Pull::Down => w.set_mode(Pio00Mode::PULL_DOWN),
        });
    }
    ...
    pub fn set_as_output(&mut self) {
        GPIO.dirset0().write(|w| w.set_dirsetp(self.bit()))
    }

    pub fn set_as_input(&mut self) {
        GPIO.dirclr0().write(|w| w.set_dirclrp(self.bit()))
    }
}
...
pub(crate) trait SealedPin: Sized {
    ...
    #[inline]
    fn pio(&self) -> Reg<iocon::regs::Pio00, RW> {
        match self.pin_bank() {
            Bank::Gpio0 => IOCON.pio0_0(),
        }
    }
}
